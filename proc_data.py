...
root_dir = '/amax/hchuz/architectural_heritage/data/分类的texture'
target_dir = '/amax/hchuz/architectural_heritage/data/分类的texture/all'
jsonl_file = '/amax/hchuz/architectural_heritage/data/分类的texture/meta.jsonl'
json_file = '/amax/hchuz/architectural_heritage/data/分类的texture/meta.json'
...
def update_jsonl_to_json(jsonl_file, json_file):
    ...
    for line in json_lines:
        if line['img_path'] not in json_dict:
            json_dict[line['img_path']] = {'texture': line['texture'], 'caption':line['caption']}
...
for texture, prefix in zip(textures, prefixs):
    img_root = os.path.join(root_dir, texture)
    ...
    for img_name in tqdm(img_names):
        ...
        #     json_line = json.dumps({'texture': texture, 'img_path': img_path, 'caption': text})  # 将对象转换为 JSON 格式的字符串
        #     f.write(json_line + '\n')  # 写入文件并换行
