    ...
    def __call__(
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                if masks != None:
                    #num_ref_img_tokens = 1241
                    assert len(masks) == noise_pred.shape[0] - 1
                    canvas = noise_pred[1:]
                    #print(canvas.shape)
                    #canvas *= masks.to(noise_pred.device, noise_pred.dtype)
                    #canvas = canvas.mean(0, keepdim=True)
                    #print(hidden_states[:1, num_ref_img_tokens:, :][masks.repeat(1,1,hidden_states.shape[-1])].shape)
                    noise_pred[:1][masks.repeat(1,1,noise_pred.shape[-1])] = canvas[masks.repeat(1,1,noise_pred.shape[-1])]
